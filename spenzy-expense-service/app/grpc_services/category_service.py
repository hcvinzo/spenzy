...
class CategoryServicer(expense_pb2_grpc.CategoryServiceServicer):
    ...
    def ListCategories(self, request, context):
        ...
        try:
            categories = self.loop.run_until_complete(
    ...
    def UpdateCategory(self, request, context):
        ...
        try:
            ...
            category = self.loop.run_until_complete(
                self.category_service.update_category(request.category_id, update_data)
            )
    ...
    def DeleteCategory(self, request, context):
        ...
        try:
            success = self.loop.run_until_complete(
                self.category_service.delete_category(request.category_id)
            )
            if not success:
